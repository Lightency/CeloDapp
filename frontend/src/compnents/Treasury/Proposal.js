...
const payProposal = async (id) => {
    await Dao.pay(id).then(console.log("done")).catch((e) => { console.log("error:" + e) });
}
const getProposal = async (id) => {
  await Dao.getProposal(id).then((res)=>{setProposal(res);}).catch((e) => { console.log(e)});
...
useEffect(() => {
  ...
  return (
    ...
    <Card.Header style={{backgroundColor:"black",color:"white",fontSize:"2em"}}>{proposal.title}</Card.Header>
    <Card.Body>
      <Card.Title>Proposal N°: {BigNumber.from(proposal.id).toString()}</Card.Title>
       ...
       <li><strong>amount</strong>{(BigNumber.from(proposal.amount).toNumber())/10**8} Lts</li> 
      ...
      <Button onClick={() =>{vote(1,proposal.id,true)}} style={{width:"25%"}} variant="success">Yes</Button>
      <Button onClick={() =>{vote(1,proposal.id,false)}} style={{width:"25%"}} variant="danger">No</Button>
